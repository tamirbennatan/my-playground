...
extract_arima_terms <- function(model){
      tmp = tidy(model) %>%
            mutate(term.type = ifelse(substring(first = 1, last = 1, term) == "s", "seasonal", "non_seasonal")) %>%
            group_by(term.type) %>%
            summarise(num.terms = n()) %>%
            spread(key = term.type, value = num.terms) 
            
      if (!("seasonal" %in% colnames(tmp))){
            tmp$seasonal <- 0
      }
      
      tmp <- tmp %>%
            mutate(order = seasonal + non_seasonal)
...
nested_sample %>%
      head(10) %>%
      select(article, arima_model, seasonal, non_seasonal, order)
