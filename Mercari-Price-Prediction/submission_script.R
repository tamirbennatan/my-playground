...
train <- read.csv("../input/train.tsv", row.names = NULL, sep = "\t")
...
category.onehot <- function(data){
      ...
      data$high_category <- factor(data$high_category, levels = high.category.levels, ordered = TRUE)
...
low.mid.categories.onehot <- function(data){
      ...
      # change to a factor
      data$mid.low.categories <- factor(data$mid.low.categories, levels = low.mid.category.levels, ordered = TRUE)
...
brand.onehot <- function(data){
      ...
      # convert to a factor 
      data$brand_name <- factor(data$brand_name, levels = brand.levels, ordered = TRUE)
...
condition.levels = c(1,2,3,4,5,-1)
...
condition.onehot <- function(data){
      ...
      # convert to factor
      data$item_condition_id <- factor(data$item_condition_id, levels = condition.levels, ordered = TRUE)
...
shipping.binary.var <- function(data){
      ...
      # convert to a factor
      data$shipping <- as.factor(data$shipping,levels = shipping.levels, ordered = TRUE)
...
print("Converting train to sparse matrix...")
...
test <- read.csv("../input/test.tsv", row.names = NULL, sep = "\t")
